...
void SumpT_vs_pT_Transverse(){
    ...
    TH2D* h = new TH2D("h","Sum pT vs pT Transverse",50,0,5,50,0,50);
	...
	double sum_pT=0;
    	for(int j=0; j<ntrks; j++) {
    		...
    		if((delphi[j]>PI/3) && (delphi[j]<2*PI/3) && (eta[j]>-2.5) && (eta[j]<2.5)){	
    			sum_pT = sum_pT + pT[j];
    		}
    	}
